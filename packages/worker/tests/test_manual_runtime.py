...
class ManualRuntimeTest(unittest.TestCase):
    ...
    def test_materialize_custom_voice_runtime_model_writes_runtime_files(self) -> None:
        spec = get_model_spec("qwen3_tts_0_6b_custom_voice")
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            config_path = output_root / "qwen3_tts_0_6b_custom_voice" / "config.pbtxt"
            model_path = output_root / "qwen3_tts_0_6b_custom_voice" / "1" / "model.py"
            self.assertTrue(config_path.exists())
            self.assertTrue(model_path.exists())
            self.assertIn('name: "qwen3_tts_0_6b_custom_voice"', config_path.read_text(encoding="utf-8"))
            self.assertIn("generate_custom_voice", model_path.read_text(encoding="utf-8"))
